# this script will read the PhysiCell output files using pcdl and plot cell cycle phase distributions over time
...
def analyze_cell_cycle_phases(output_dir):
    """Analyze cell cycle phase distributions over time using pcdl"""
    ...
    try:
        ...
        if hasattr(ts, 'timesteps'):
            ...
            # Try to get time points from the data
            print("Attempting to extract time points from data...")
            time_points = []
            # We'll try to get time points from the available data
            try:
                # Get the first available time point to check structure
                first_time = list(ts.data.keys())[0] if hasattr(ts, 'data') else None
                if first_time:
                ...
                print("Could not extract time points")
                return None, None
        ...
        for timestep in time_points:
            try:
                ...
                phase_column = None
                for col in ['current_phase', 'phase', 'cell_phase', 'cycle_phase']:
                ...
                print(f"Time {timestep}: {len(cell_df)} cells, data: {phase_distribution}")
...
def plot_phase_percentages(phase_counts, time_points, output_path):
    ...
    ax.stackplot(time_points, 
                [phase_percentages[phase] for phase in all_phases],
                labels=[f'Phase {phase}' for phase in all_phases],
                colors=colors,
                alpha=0.7)
...
if __name__ == "__main__":
    ...
    if phase_counts and time_points:
        ...
        print("3. Cell data contains 'current_phase' column")
